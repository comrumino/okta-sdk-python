...
async def test_response_pagination_with_next_include_response(monkeypatch):
    ...
    result, error = await ssws_client.get_request_executor().execute(req, models.Group)
    ...
    monkeypatch.setattr(RequestExecutor, 'fire_request',
                        mocks.mock_GET_HTTP_Client_response_valid)
    res, err, resp = await result.next(includeResponse=True)
    assert isinstance(res[0], models.Group)
    assert err is None
    assert resp._type is models.Group
